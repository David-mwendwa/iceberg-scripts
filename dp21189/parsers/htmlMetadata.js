async function parsePage({URL, responseBody, referer}) {
    ...
    let Tipo_de_documento = /propuesto\s+(Proyecto\sde\s(?:decreto|resolución|otros|minuta))/i.exec(detailText);
    Tipo_de_documento = Tipo_de_documento && Tipo_de_documento[1];
    const sector = $(`td:has(span[id *='outSector'])`).text();

    const Fecha_de_inicio_spanish = $(`td:has(span[id *='outFecha'])`).first().text();
    let d = Fecha_de_inicio_spanish && Fecha_de_inicio_spanish.replace(/\sdel?/gi, '');
    d = moment(d, 'DD MMMM YYYY', 'es');
    let Fecha_de_inicio = d && d.isValid() ? d.format('YYYY-MM-DD') : null;

    const Fecha_Fin_spanish = $(`td:has(span[id *='outFechaFin'])`).first().text();
    d = Fecha_Fin_spanish && Fecha_Fin_spanish.replace(/\sdel?/gi, '');
    d = moment(d, 'DD MMMM YYYY', 'es');
    let Fecha_Fin = d && d.isValid() ? d.format('YYYY-MM-DD') : null;

    let summario = $(`span[id *='outDetalle'] > p:contains("Documento propuesto")`).next().next().text().trim();
  	if (!summario) {
    	summario = $(`span[id *='outDetalle'] > p:last-child`).text().trim()
    }
  	if (summario.length < 30) {
    	summario = $(`span[id *='outDetalle'] > p:contains("Documento propuesto")`).next().text().trim();
    }
  	let mother = {
      	isMother: true, URI: [URL], title, sector, 
    ...
    let href = propuesto.find('a[href*="docx"], a[href*="pdf"], a[href*="doc"]').attr('href');
  	href = href ? url.resolve(URL, href) : null;
    title = propuesto.find('a[href*="docx"], a[href*="pdf"], a[href*="doc"]').text()
  	if (!href) {
        propuesto = $('p:contains("Documento propuesto: "), span:contains("Documento propuesto")');
        href = propuesto.find('a[href*="docx"], a[href*="pdf"], a[href*="doc"]').attr('href');
      	href = href ? url.resolve(URL, href) : null;
        title = propuesto.find('a[href*="docx"], a[href*="pdf"], a[href*="doc"]').text();
    }
  	if (!href) {
    ...
    href && results.push({URI: href, parentURL: URL, title, documentType: "Documento propuesto", sortValue, inverseSortValue, Class })

    // Otros documentos
    let otrosEvents = [];
    let otros = container.find('p:contains("Otros Documentos")').next();
    otros.find('a[href*="docx"], a[href*="pdf"], a[href*="doc"]').each(function () {
    ...
    for (let i = 0; i < otrosEvents.length; i++) {
      ...
      //if (inverseSortValue === 1) {Class = 'initial bill'} else {Class = null} 
      href && results.push({URI: href, parentURL: URL, title, documentType: "Otros documentos", sortValue, inverseSortValue, Class})
    }

    // Conclusiones
    let concEvents = [];
    let conc = container.find('[id $="conclusiones"]');
    conc.find('a[href*="docx"], a[href*="pdf"], a[href*="doc"]').each(function () {
    ...
    for (let i = 0; i < concEvents.length; i++) {
      ...
      //if (inverseSortValue === 1) {Class = 'initial bill'} else {Class = null} 
      results.push({URI: href, parentURL: URL, title, documentType: "conclusiones", sortValue, inverseSortValue, Class})
    }
