...
function parseFormData(curlString) {
    const text = /--data-binary +\$?['"](.+)['"]/i.exec(curlString)[1];
    let boundary = "-{20,}\\d{5,}[-]*";
...
const curlContentParser = function ({curlString, requestIndex, functionPrefix = "method"}) {
    ...
    if (/--data\s+/i.test(curlString)) {
        ...
        let x = parseForm(curlString);
        [method, string] = [x.method, x.string];
    } else if (/--data-binary\s+/i.test(curlString)) {
...
const generateCode = function () {
    ...
    let FILE = __dirname + "/./pdf/dp0160-bis.txt"; //set this to target file, with curl requests each on a new line
