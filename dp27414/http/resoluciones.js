...
async function puppeteerFetch({canonicalURL, headers}) {
  	let match = /dependencia=(.+?)&year=(.+?)&page=(.+)/.exec(canonicalURL)
    let dependencia = match && match[1].replace(/-/g, ' ')
    let year = match && match[2]
    let pageNo = match && match[3]
    let titlegroupstring = encodeURI(`;#${dependencia};#`).replace(/;#/g, "%3B%23").replace(/,/g, '%2C').replace(/%252C/g, "%2C")
  	let docgroupstring = encodeURI(`;#${dependencia};#${year};#`).replace(/;#/g, "%3B%23").replace(/,/g, '%2C').replace(/%252C/g, "%2C")
  	let requestURL = canonicalURL.replace(/\?.*/g, '')
  	//throw(JSON.stringify({dependencia, year, pageNo, canonicalURL, docgroupstring, titlegroupstring}, null, 4))
    ...
    await page.goto(requestURL,{waitFor:"networkidle0", timeout:60000}).catch(e=>console.error(`Puppeteer still loading page ${canonicalURL}`));
  	await page.waitForSelector(`tbody[groupstring*="${titlegroupstring}"] a`);
  	await page.click(`tbody[groupstring*="${titlegroupstring}"] a`);
...
async function fetchURL({canonicalURL, headers}) {
    if (/https?:.*https?:/i.test(canonicalURL)) {
  	...
  	const isHome = canonicalURL.match(/Resoluciones\.aspx$/)
  	const isResoluciones = canonicalURL.match(/Resoluciones\.aspx\?dependencia=(.+?)&year=(.+?)&page=(.+)/)
    if (isHome) {
    	return [await _puppeteerFetchHome({canonicalURL, headers})]
    }
    if (isResoluciones) {
        let year = isResoluciones[1]
